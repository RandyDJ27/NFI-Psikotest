...
elseif ($module=='users' AND $act=='hapus') {
	mysqli_query($koneksi, "DELETE FROM tbl_user WHERE id_user='$_GET[id]'");
  header('location:../../media.php?module='.$module);
}
elseif ($module=='users' AND $act=='input'){
  ...
  if (!empty($lokasi_file)){
    ...
    move_uploaded_file($lokasi_file,"foto/$nama_file");
	  ...
	  mysqli_query($koneksi, "INSERT INTO user(nama,pangkat,nrp,password,jabatan,gambar,level) 
	  VALUES('$_POST[nama]','$_POST[pangkat]','$_POST[nrp]','$_POST[password]',
	  '$_POST[jabatan]','$nama_file','user')");
  }
  else {
	  mysqli_query($koneksi, "INSERT INTO user(nama,pangkat,nrp,password,jabatan,level) 
	  VALUES('$_POST[nama]','$_POST[pangkat]','$_POST[nrp]','$_POST[password]',
	  '$_POST[jabatan]','user')");
  }
  header('location:../../media.php?module='.$module);
...
$aktif='N';
    mysqli_query($koneksi, "UPDATE tbl_user SET statusaktif  = '$aktif'  WHERE id_user='$_GET[id]'");
  header('location:../../media.php?module='.$module);
